...
# The only changes in this file are to set the random seed to global rank and to comment out the write to cache file.
...
rank = int(os.environ['RANK'])   ### Define global rank.
...
class Pipeline:
    ...
    def train(self,
        ...
        set_seed(rank)                               ### Set seed to global rank.
...
def build_dataset(configs: dict,
    ...
    if cache_file.exists():
        ...
        raw_data = data['raw_data']
        # store cache
#        with open(cache_file, 'wb') as f:                  ### Disable write to cache file.
#            pickle.dump(data, f)                           ### Disable write to cache file.
