...
# This code does not need to be parallelized.  The split between training and testing data is re-structured.
# The random seed is the global rank.

import os                            # random seed from global rank
rank = int(os.environ['RANK'])       # random seed from global rank
...
class MATRPrimaryTestTrainTestSplitter(MATRTrainTestSplitter):
    def __init__(self, cell_data_path: str):
        ...
        test_set.pop(train_set.index('b2c1'))

        random.seed(rank)                               # random seed from global rank
        train_ids = random.sample(train_set, 100)       # random seed from global rank and choose 100 data for training
        test_ids = random.sample(test_set, 60)          # random seed from global rank and choose 60 data for testing
...
@TRAIN_TEST_SPLITTERS.register()
class MATRSecondaryTestTrainTestSplitter(MATRPrimaryTestTrainTestSplitter):
    pass
#class MATRSecondaryTestTrainTestSplitter(MATRTrainTestSplitter):


@TRAIN_TEST_SPLITTERS.register()
class MATRCLOTestTrainTestSplitter(MATRPrimaryTestTrainTestSplitter):
    pass
